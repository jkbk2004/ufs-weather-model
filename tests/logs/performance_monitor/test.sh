...
def extract_baseline_dates(conf_path="bl_date.conf"):
    """
    Parses baseline update dates from bl_date.conf
    """
    dates = []
...
def extract_wall_times(log_dir=".", conf_path="bl_date.conf"):
    """
    Parses all RegressionTests_*.log files and builds a time series dataset
    with anomaly detection and baseline annotations.
    """
