...
def normalize_resource_values(info):
    """
    Convert all numeric resource values into strings suitable for YAML and XML.
    - wlclk integers are interpreted as minutes → HH:MM:SS
    - all other numeric values are stringified
    """
    normalized = {}
    for key, value in info.items():
        if key == "wlclk":
            if isinstance(value, int):
                # Interpret integer as minutes
                normalized[key] = f"00:{value:02d}:00"
            else:
                normalized[key] = str(value)
        else:
            # Convert all other numeric values to strings
            normalized[key] = str(value)
    return InlineDict(normalized)
        ...
        for test_entry in tests:
            for test_name, test_config in test_entry.items():
                ...
                for platform in platforms:
                    ...
                    if info:
                        # Normalize values before writing to YAML
                        resources[platform] = normalize_resource_values(info)
